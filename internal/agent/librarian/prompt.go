...
要求：
1. 从评估对话中找出修行者最薄弱的 1~2 个方向，针对每个方向生成一个任务；
2. 任务必须具体可执行，不能是"学习X"、"了解Y"这类模糊指令，应是"用X实现Y"、"阅读Z并写出运行结果"这类有明确产出的动作；
3. acceptance_criteria（验收标准）必须可被客观判断，例如"能口述三次握手每步的状态机"、"提交一段使用pprof分析内存的完整代码"；
4. category 必须是以下之一：go_lianqi / ai_lianqi / wufu；
5. source_evidence 填写触发此任务的具体弱点描述（一句话）；
6. target_score_hint 填写预计完成后该方向分数可提升的幅度（整数，如 5）；
7. 待完成任务已超过 2 个时，返回空数组 []；
8. 只输出 JSON 数组，不要任何包裹文字、代码块标记或解释。
...
func buildTaskExtractPrompt(userName, conversation, profileSummary string, pendingCount int) string {
	return fmt.Sprintf(
		"修行者（%s）刚完成了一次评估，当前档案如下：\n%s\n\n本次完整对话：\n%s\n\n当前待完成任务数量：%d（超过2个时请返回空数组[]）\n\n请根据以上信息生成修炼任务：",
		userName, profileSummary, conversation, pendingCount,
	)
}
