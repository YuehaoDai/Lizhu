...
const sessionSummarizePrompt = `你是骊珠修行体系的「知识整理官」，负责将一次修行对话提炼为简短的会话摘要，供护道人日后回顾。

要求：
1. 用一句话或两句话概括本次对话的核心内容（修行者问了什么、做了什么、护道人给出了什么要点）；
2. 不超过 80 字；
...
// buildSessionSummarizePrompt 构建对话摘要的用户消息。
func buildSessionSummarizePrompt(userName, userInput, reply string) string {
	return fmt.Sprintf("修行者（%s）与护道人的对话如下，请提炼摘要：\n\n【修行者】\n%s\n\n【护道人】\n%s",
		userName, userInput, reply)
}
